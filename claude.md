...
- [feat] **Hybrid Raw-Food Recommendation Algorithm (Sprint 6):** Hoàn thiện trải nghiệm người dùng với tính năng Smart Swap và Ghi Nhật Ký. Xây dựng Modal đổi món cho phép truy vấn trực tiếp kho dữ liệu Raw Food theo category. Tích hợp tính năng Re-calculate (gọi Module 3 lại) sau khi Swap. Viết logic Client để đồng bộ 4 nguyên liệu thô vào Nhật ký ăn uống (Diary) thông qua Promise.all() chỉ với 1 click, tự động scale số gram hợp lệ theo API `amount`.
- [feat] **Thuật toán TDEE Thích ứng (Adaptive TDEE):** Triển khai cơ chế phân tích dữ liệu cân nặng (EMA - lọc nhiễu) và dữ liệu lượng calo nạp vào thực tế trong tuần để tính toán lại TDEE thực, phá vỡ giới hạn của công thức tĩnh Mifflin. Tự động CLAMP trong khoảng an toàn (±30%) và dùng trung bình động (Rolling Average) 4 tuần. Bao gồm Cron Job tự động (node-cron chạy sáng Thứ 2) và tuỳ chọn Skip Week/Toggle trong Profile. UI Dashboard được tích hợp badge thông báo và tooltip.
